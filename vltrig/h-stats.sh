...
stats_raw=`curl --connect-timeout 2 --max-time $API_TIMEOUT --silent --noproxy '*' http://127.0.0.1:$CUSTOM_API_PORT/api.json`
if [[ $? -ne 0 || -z $stats_raw ]]; then
  ...
  local gpu_bus_ids=`cat ${CUSTOM_LOG_BASENAME}_head.log | grep '|..[[:digit:]] |...[[:digit:]] | ' | awk '{ printf $6"\n" }' | cut -d ':' -f 1`
